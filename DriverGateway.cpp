...
void DriverGateway::retrieveOrderHistory(Driver driver) {
...
void DriverGateway::displayCarInf(Driver driver) {
    if (!checkAuth(driver)) {return;}
...
void DriverGateway::dispAvOrders(Driver driver) {
    if (!checkAuth(driver)) {return;}
...
void DriverGateway::takeOrder(Driver driver, Passenger passenger ,int orderId) {
    ...
    cout << "Driver " << driver.getName() << " took this order" << endl;
